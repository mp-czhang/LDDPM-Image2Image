...
def read_raw_image(filepath, height, width, dtype=np.uint16):
...
class PairedImageDataset(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if isinstance(self.splits[datasets.Split.TRAIN], int): # Ratio-based selection     
            all_images = [img for img in sorted(os.listdir(os.path.join(local_path, 'imageA'))) if img.endswith('.raw')]
        ...
        return [
            datasets.SplitGenerator(
                name=split_name,
                gen_kwargs={
                    'imageA_dir': os.path.join(local_path, 'imageA'),
    ...
    def _generate_examples(self, imageA_dir, imageB_dir, start_idx, end_idx):    	
        ...
        for img_filename in selected_images:
            if img_filename.endswith('.raw'):
                # use the first (L/F) letter to differentiate labels
                pair_name = img_filename[1:]
                imgA_filepath = os.path.join(imageA_dir, f"L{pair_name}")             
                imgB_filepath = os.path.join(imageB_dir, f"F{pair_name}")
                imgA_data = read_raw_image(imgA_filepath, self.img_height, self.img_width)
                imgB_data = read_raw_image(imgB_filepath, self.img_height, self.img_width)               
